...
export default function AdminNews() {
  ...
  const fetchNews = async () => {
    try {
      ...
      setNews(data.filter((item: NewsItem) => item.isTender === false));
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast({ title: "Success", description: `News ${editingItem ? "updated" : "added"} successfully` });
        setModalOpen(false);
        fetchNews();
      } else {
  ...
  return (
    <AdminLayout title="Manage Latest News">
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            <DialogTitle>{editingItem ? "Edit News" : "Add New News Item"}</DialogTitle>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label htmlFor="title">News Title <span className="text-red-500">*</span></Label>
              <Textarea id="title" value={formData.title} onChange={(e) => setFormData({...formData, title: e.target.value})} rows={3} />
            </div>
            <div className="grid gap-2">
              <Label htmlFor="lastDate">Last Date (if applicable)</Label>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave} disabled={saving || !formData.title} className="bg-[#1a3a6e] hover:bg-[#1a3a6e]/90 text-white">
