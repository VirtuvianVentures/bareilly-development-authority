...
import { Plus, Pencil, Trash2, X, Loader2, TrendingDown } from "lucide-react";
...
export default function DeductionMaster() {
  ...
  const load = async () => {
    setLoading(true);
    try { const r = await apiFetch("/deduction-heads"); setHeads(await r.json()); }
  ...
  const save = async () => {
    ...
    try {
      if (editItem) {
        await apiFetch(`/deduction-heads/${editItem.id}`, { method: "PUT", body: JSON.stringify({ name: name.trim(), amount: Number(amount) || 0, isActive: true }) });
        toast({ title: "Deduction head updated" });
      } else {
        await apiFetch("/deduction-heads", { method: "POST", body: JSON.stringify({ name: name.trim(), amount: Number(amount) || 0 }) });
        toast({ title: "Deduction head added" });
      }
  ...
  const remove = async (h: Head) => {
    ...
    await apiFetch(`/deduction-heads/${h.id}`, { method: "DELETE" });
  ...
  return (
    <PayrollLayout title="Deduction Master">
      <div className="max-w-3xl mx-auto">
        {/* Header */}
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="text-lg font-bold text-gray-800">Deduction Heads</h2>
            <p className="text-sm text-gray-500">{heads.length} heads &nbsp;·&nbsp; Total: <span className="font-semibold text-red-600">₹{total.toLocaleString("en-IN")}</span></p>
          </div>
          <Button onClick={openAdd} className="bg-red-600 hover:bg-red-700 gap-2">
            <Plus className="h-4 w-4" /> Add Deduction Head
          </Button>
        </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {heads.length === 0 ? (
                  <tr><td colSpan={5} className="text-center py-16 text-gray-400">
                    ...
                    <p className="text-sm">No deduction heads. Click <span className="text-red-600 font-semibold">Add Deduction Head</span> to start.</p>
                  </td></tr>
                ) : heads.map((h, i) => (
                  <tr key={h.id} className="hover:bg-red-50/30 transition-colors">
                    <td className="px-4 py-3 text-gray-500">{i + 1}</td>
                    <td className="px-4 py-3 font-medium text-gray-800">{h.name}</td>
                    <td className="px-4 py-3 text-right font-mono font-semibold text-red-600">₹{Number(h.amount).toLocaleString("en-IN")}</td>
                ...
                {heads.length > 0 && (
                  <tr className="bg-red-50 font-bold">
                    ...
                    <td className="px-4 py-3 text-right text-red-600">₹{total.toLocaleString("en-IN")}</td>
                    <td colSpan={2} />
                  </tr>
                )}
              </tbody>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b bg-red-700 rounded-t-xl">
              <h3 className="font-bold text-white">{editItem ? "Edit Deduction Head" : "Add Deduction Head"}</h3>
            ...
            <div className="px-6 py-5 space-y-4">
              <div>
                ...
                <Input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. GPF, Income Tax, NPS, LIC" className="mt-1" autoFocus onKeyDown={e => e.key === "Enter" && save()} />
              </div>
              <div>
